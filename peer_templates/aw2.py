...
##  useful to 'pretty print, or pretty format' data-structures, that are nested
##  or not.   
from pprint import pprint  

## Import the 'Template' class from the 'string' module. This is useful for
##   modifying code using '$' prefixes for variables used as text substitutions
##   when constructing a new string from a given string.
from string import Template

import argparse  	## module to get command arguments from the user.
import ipaddress  	## module with many methods to create and control IPV4/IPv6
...
## Initializes 'banner' as a multi-line string.
banner = """ 
###############################################################################
#    ____ _                 _                                                 #
#   / ___| | ___  _   _  __| |                                                #
...
#                                                                             #
###############################################################################
""" # Creator: patorjk.com/software/taag Font: Standard

## Create DEPS as a list of strings.
##   The strings are names of some of the modules this program depends on.
## 
## In the list of modules is python-terraform that wraps the terraform command.
##   A perhaps more pythonic module, terraformpy is available for this purpose.
DEPS = [
    'colorama',
    'gitpython',
    'netaddr',
...
## is to exit this program if it fails to import the packages listed in 'DEPS', but first print two
## messages telling the user to install python packages listed in DEPS. 
try: 
    from python_terraform import *  ## A module to call the 'terraform' command.
                                    ## PROGRAMMER RAISED_EYEBROW WARNING: this imports '*' which is substandard and error prone.
                                    ## With '*', all python_terraform functions, classes and variables are imported into this
                                    ## program's namespace.  Any collision of variable names would override the variables
                                    ## defined in this code.  This importing of * increases complexity of maintaining
                                    ## the code in this program, and could increase complexity of debugging it.
                                    ## See: https://www.geeksforgeeks.org/why-import-star-in-python-is-a-bad-idea/
    ...
    from termcolor import colored  ## A module for ANSII Color formatting output for terminals of specific type based on shell exported $TERM  
                                   ## The code could be simplified a little if the 'cprint' function were also imported. 

    from git import Repo           ## A package of modules to interact with git repositories.

    from netaddr import IPNetwork  ## A package of modules to manipulate IPv4/IPv6 networks.

    import colorama                ## Makes ANSI escape character sequences (for producing colored terminal text and cursor
                                   ## positioning) work under MS Windows.

## If a ImportError exception is encountered during execution of any above code within the 'try' block section above,
## then print instructions for the user telling them what to install, and exit this program with a 0 command return code.
except ImportError:
    print()
    print('Install the following dependencies and try again:', 'red')  ## PROGRAMMER WARNING: This looks like a programming defect, The call to print()
                                                                       ## looks as though it should have called termcolor.cprint() to print in 'red'.
    print('sudo pip3 install -t /opt/python3/lib/python3.7/site-packages/')  ## Tells the user what to install:

    ## Loop through the DEPS list as 'dep', telling the user each "- dep" they should install.
    for dep in DEPS:
        print('- ' + dep)
    
    exit()  ## exit from this program with a 0 command return code. This would indicate command SUCCESS.


colorama.init()  ## Initialize colorama, so colored text will work on MS Windows systems.

## Defines 'Arguments' as a program class with the default Python class type of 'object'.
